...
async def add_content(
    ...
    existing_content = (
        db.query(Content)
        .filter(Content.cid == new_content.cid, Content.owner_id == user.id)
        .first()
    )
    ...
    content = Content(
        cid=new_content.cid, filesize=new_content.filesize, owner_id=user.id
    )
...
async def list_crust_orders(
    ...
    if content_in.id:
        content = (
            db.query(Content)
            .filter(Content.id == content_in.id, Content.owner_id == user.id)
            .first()
        )
    else:
        content = (
            db.query(Content)
            .filter(Content.id == content_in.cid, Content.owner_id == user.id)
            .first()
        )
