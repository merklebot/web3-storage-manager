...
class Web3Uploader:
    ...
    def check_not_uploaded_content(self, db: Session):
        while True:
            ...
            content_for_upload = (
                db.query(Content).filter(~Content.crust_orders.any()).all()
            )
            for content in content_for_upload:
                tips = 0
                _memo = ""

                is_success, extrinsic_hash, block_number = make_storage_order(
                    cid=content.cid, filesize=content.filesize, tips=tips, _memo=_memo
                )
                if is_success:
